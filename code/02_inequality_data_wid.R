...
data_incshare_pretax <- download_wid(
  ...
  perc = c(
    ...
    "p99.99p100",   # Top 0.01%
    "p0p10"),       # Bottom 10%
  metadata = TRUE, ages = 992, pop = "j") #all ages// pop- equally split between adults
...
data_incshare_pretax <- dcast(data_incshare_pretax, country + countryname + year + shortname ~ percentile, value.var = "sptinc992j")
...
data_incshare_dispinc <- download_wid(
  ...
  perc = c(
    ...
    "p99.99p100",   # Top 0.01%
    "p0p10"),       # Bottom 10%
  metadata = TRUE, ages = 992, pop = "j") # ages 20+ (adults, including elderly)// pop- equally split between adults
...
data_incshare_dispinc <- dcast(data_incshare_dispinc, country + countryname + year +shortname ~ percentile, value.var = "sdiinc992j")
...
gini_di <- download_wid(
  ...
  years = 1960:2024,        
  perc = "p0p100",
  ages = 992,               # ages 20+ (including elderly)
  metadata = TRUE, 
  pop = "j")                # equally split between adults
...
setnames(ineq, old = c("gdiinc992j", "gptinc992j"),
